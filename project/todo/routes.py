...
@users_blueprint.route("/all_tasks")
@login_required
def all_tasks():
...
@users_blueprint.route("/add_task", methods=['POST', 'GET'])
@login_required
def add_task():
    form = TaskForm()
    if form.validate_on_submit():
...
@users_blueprint.route("/all_tasks/<int:task_id>/update_task", methods=['GET', 'POST'])
@login_required
...
@users_blueprint.route("/all_tasks/<int:task_id>/delete_task")
@login_required
...
def account():
    ...
    if form.validate_on_submit():
        if form.username.data != current_user.username:  
            ...
            return redirect(url_for('todo.account'))
    elif request.method == 'GET':
...
@users_blueprint.route("/account/change_password", methods=['POST', 'GET'])
@login_required
def change_password():
